  ...
  const handleSignIn = async () => {
    try {
      ...
      if (error) {
        ...
        console.log("Successfully signed in with Google:", session);
      }
    } catch (error) {
    ...
    <div className="w-screen h-screen flex justify-center items-center">
      <div className="flex justify-center mt-10 rounded-2xl bg-indigo-700 border-4 border-sky-600 shadow-2xl shadow-sky-500 w-[50%] h-80">
        <div className="flex flex-col justify-start items-center">
          ...
          <div onClick={handleSignIn} className="mt-20  cursor-pointer bg-sky-600 p-5 w-[150%] flex justify-center rounded-3xl">
            <h3 className="text-white font-bold text-xl">Sign in with Google!</h3>
          </div>
        </div>
      </div>
    </div>
